...
class map2D(object):
    ...
    def set_particle(self, npart=1, emit=1e-6, twiss_beam=None, espr=0.0, particles=None):
        ...
        if particles is None:
            ...
            self.arrayxy=np.zeros((2,npart))
            self.arrayxy[0, :]=np.random.randn(npart)
            avex=np.average(self.arrayxy[0, :])
            sizex=np.std(self.arrayxy[0, :])
            self.arrayxy[0, :] -= avex
            self.arrayxy[0, :] *= (np.sqrt(emit*self.beam_beta)/sizex)

            self.arrayxy[1, :] = np.random.randn(npart)
            avexp = np.average(self.arrayxy[1, :])
            sizexp = np.std(self.arrayxy[1, :])
            self.arrayxy[1, :] -= avexp
            self.arrayxy[1, :] *= (np.sqrt(emit / self.beam_beta) /sizexp)

            cxxp=np.average((self.arrayxy[0, :])*(self.arrayxy[1, :]))
            emit=np.sqrt(sizex * sizexp * sizex * sizexp - cxxp * cxxp)


            self.arrayxy[1, :]+= self.arrayxy[0, :] * ((-self.beam_alpha+cxxp/emit)/ self.beam_beta)


        else:
            self.npart = particles.shape[1]
            self.arrayxy=particles
    ...
    def propagate(self):
        if self.espread is None:
            self.arrayxy=self.linmap @ self.arrayxy
        else:
            self.arrayxy[0, : ], self.arrayxy[1, : ] = self.chrom_m11*self.arrayxy[0, : ]+self.chrom_m12*self.arrayxy[1, : ],self.chrom_m21*self.arrayxy[0, : ]+self.chrom_m22*self.arrayxy[1, : ]
    ...
    def statistics(self):
         ...
         sizex = np.std(self.arrayxy[0, :])
         sizexp = np.std(self.arrayxy[1, :])
         cxxp= np.average((self.arrayxy[0, :]-avex)*(self.arrayxy[1, :]-avexp))
